...
for file in "$inputs_folder"/*; do
  if [[ ! $(basename "$file") =~ ^$root_tag ]]; then c
    rm -f "$file"
  fi
done
...
python util/merge_batches.py -r $output_folder -s sam -o $output_folder
...
python sbir/sbir_scraper.py -i $inputs_folder/$input_file -s 0 -o $output_folder
...
python util/merge_batches.py -r $output_folder/sbir_batches -s sbir_awards  -o $output_folder
python util/merge_batches.py -r $output_folder/sbir_batches -s sbir_company_info -o $output_folder
...
python usas/usas_api.py -i $inputs_folder/$input_file -o $output_folder
echo "USAS scraper completed" >> $general_log

#16. Running Batch Merger for USAS
echo "Starting batch merger for USAS" >> $general_log
python util/merge_batches.py -r $output_folder/usas_batches -s usas_grants  -o $output_folder
python util/merge_batches.py -r $output_folder/usas_batches -s usas_contracts  -o $output_folder
python util/merge_batches.py -r $output_folder/usas_batches -s usas_company_info -o $output_folder
